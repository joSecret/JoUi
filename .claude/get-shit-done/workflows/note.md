...
- **Global scope**: `/Users/josecret/Astro/joui/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/josecret/Astro/joui/.claude/notes/`)
...
2. Glob `/Users/josecret/Astro/joui/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/josecret/Astro/joui/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/josecret/Astro/joui/.claude/notes/` — works in any directory
